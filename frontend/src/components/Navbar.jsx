...
const LogoutModal = ({ onConfirm, onCancel }) => (
  <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4">
    ...
    />
    {/* Dialog */}
    <div className="relative bg-white rounded-2xl shadow-2xl p-6 w-full max-w-sm mx-auto border border-slate-100 animate-fade-in-up">
      <div className="flex flex-col items-center text-center gap-4">
        <div className="w-14 h-14 bg-red-50 rounded-full flex items-center justify-center">
          <span className="material-symbols-outlined text-red-500 text-3xl">logout</span>
        </div>
        <div>
          <h2 className="text-lg font-extrabold text-slate-800 mb-1">Log out?</h2>
          <p className="text-sm text-slate-500">Are you sure you want to log out of SahaYatra?</p>
        </div>
        <div className="flex gap-3 w-full pt-1">
          ...
          <button
            onClick={onConfirm}
            className="flex-1 py-2.5 rounded-xl bg-red-500 hover:bg-red-600 text-white font-bold text-sm transition-all shadow-md shadow-red-500/20"
          >
            Log Out
          </button>
        </div>
...
const Navbar = () => {
  ...
  // Decode token whenever location changes (catches post-login/post-switch updates)
  useEffect(() => {
  ...
  const handleLogoutRequest = () => {
    setMobileMenuOpen(false);   // close hamburger if open
  ...
  const handleSwitchRole = async () => {
    ...
    try {
      ...
      navigate(decoded.role === 'driver' ? '/post-ride' : '/search-rides');
    } catch (err) {
  ...
  return (
    <>
      {/* Logout confirmation modal (rendered outside nav flow) */}
      {showLogoutModal && (
      ...
      <header className="bg-white/80 backdrop-blur-md font-manrope antialiased tracking-tight sticky top-0 z-50 border-b border-slate-100 shadow-sm">
            ...
            {user ? (
              <>
                <button
                  ...
                  disabled={switching}
                  className="flex items-center gap-2 px-4 py-2 bg-slate-100 hover:bg-slate-200 text-slate-700 text-sm font-bold rounded-lg transition-all disabled:opacity-60"
                >
                ...
                <button
                  ...
                  className="p-2 text-slate-600 hover:bg-red-50 hover:text-red-500 rounded-lg transition-all"
                  title="Logout"
                >
        ...
        {mobileMenuOpen && (
          <div className="md:hidden bg-white border-t border-slate-100 px-4 py-4 space-y-1 shadow-lg">
              ...
              <>
                <Link className={mobileLinkStyle('/search-rides')} to="/search-rides">
                  <span className="flex items-center gap-3">
                    <span className="material-symbols-outlined text-[20px] text-emerald-600">search</span>
                    Find Rides
                  </span>
                </Link>
                {user && (
                  <>
                    <Link className={mobileLinkStyle('/my-bookings')} to="/my-bookings">
                      <span className="flex items-center gap-3">
                        <span className="material-symbols-outlined text-[20px] text-emerald-600">directions_car</span>
                        Active Rides
                      </span>
                    </Link>
                ...
                {user && (
                  <>
                    <Link className={mobileLinkStyle('/manage-rides')} to="/manage-rides">
                      <span className="flex items-center gap-3">
                        <span className="material-symbols-outlined text-[20px] text-emerald-600">manage_accounts</span>
                        Active Rides
                      </span>
                    </Link>
                    <Link className={mobileLinkStyle('/history')} to="/history">
            ...
            {/* Auth Buttons / Switch Role / Logout */}
              ...
              {user ? (
                <>
                  <button
                    onClick={handleSwitchRole}
                    disabled={switching}
                    className="w-full flex items-center justify-center gap-2 px-4 py-3 bg-slate-100 hover:bg-slate-200 text-slate-700 font-bold rounded-xl transition-all disabled:opacity-60"
                  >
                    <span className="material-symbols-outlined text-[18px]">swap_horiz</span>
                    {switching ? 'Switching…' : `Switch to ${user.role === 'passenger' ? 'Driver' : 'Passenger'}`}
                  </button>
